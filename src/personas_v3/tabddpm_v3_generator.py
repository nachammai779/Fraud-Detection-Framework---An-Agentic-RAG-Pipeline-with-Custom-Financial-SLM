"""
V3 generator — persona-conditioned Tab-DDPM-style sampling with:
...
Reads: datasets_v3/{archetype}/personas/persona_profiles.json
       datasets_v3/sources/typology_registry.json
Writes: datasets_v3/{archetype}/synthetic/transactions.parquet

Usage:
  python src/personas_v3/tabddpm_v3_generator.py --all --samples_per_archetype 5000
"""
...
def sample_persona(persona: dict, n: int, archetype: str, rng: np.random.Generator) -> pd.DataFrame:
    ...
    df = pd.DataFrame({
        ...
        "id": [f"v3_{archetype}_{uuid.uuid4().hex[:10]}" for _ in range(n)],
        "archetype": archetype,
        "source": "tabddpm_v3_persona_grounded",
...
def generate_archetype(archetype: str, samples: int, rng: np.random.Generator) -> pd.DataFrame:
    doc = json.loads((V3 / archetype / "personas" / "persona_profiles.json").read_text(encoding="utf-8"))
