"""
Export v4 dataset bundle — mirrors src/personas/export_dataset.py shape.
...
def _load_archetype_transactions(arch: str) -> tuple[pd.DataFrame, str]:
    ...
    adapted = V4 / arch / "adaptive" / "transactions_adapted.parquet"
    if adapted.exists():
        df = pd.read_parquet(adapted)
        return df, "narrative_filled"
    raw = V4 / arch / "synthetic" / "transactions.parquet"
...
def export_transactions():
    ...
    combined.to_parquet(EXPORTS / "transactions_v4_20k.parquet", index=False)
    combined.to_csv(EXPORTS / "transactions_v4_20k.csv", index=False)
    print(f"  transactions_v4_20k.parquet ({len(combined)} rows)")
    print(f"  transactions_v4_20k.csv")
...
def export_personas():
    ...
    for arch in ARCHETYPES:
        p = json.loads((V4 / arch / "personas" / "persona_profiles.json").read_text(encoding="utf-8"))
...
def export_dataset_card(coverage: dict, combined: pd.DataFrame):
    sources = json.loads((V4 / "sources" / "sources.json").read_text(encoding="utf-8"))
    ...
    card = f"""# Persona-Conditioned Fraud Detection Dataset (v4, Expanded Typology Coverage)

## What's new in v4 vs v3

v4 upgrades the typology coverage of v3. Sixteen persona edits expand documented fraud events, the typology resolver prefers SAR advisories over FTA codes when both match, and narratives are inherited from v3 by persona_id. Every persona field
...
| File | Description |
|------|-------------|
| `transactions_v4_20k.parquet` | Combined 20,000-row dataset across 4 archetypes |
| `transactions_v4_20k.csv` | CSV version |
...
Adaption Labs — narrative fill inherited from v3 by persona_id overlay.
...
def main():
    ...
    print("Exporting v4 dataset bundle...")
