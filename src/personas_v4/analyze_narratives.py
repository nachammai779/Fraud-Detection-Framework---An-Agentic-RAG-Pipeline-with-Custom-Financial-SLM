...
Writes a summary to datasets_v4/exports/analysis_report.json and prints a
...
def _load_v3_all() -> pd.DataFrame:
    ...
    for arch in ARCHETYPES:
        df = pd.read_parquet(V4 / arch / "adaptive" / "transactions_adapted.parquet")
        frames.append(df)
...
def _load_personas() -> dict[str, dict]:
    out = {}
    for arch in ARCHETYPES:
        doc = json.loads((V4 / arch / "personas" / "persona_profiles.json").read_text(encoding="utf-8"))
...
def q5_v2_vs_v3(df: pd.DataFrame) -> dict:
    ...
    # V4 structural checks — compute per-bucket "how many rows violate this concern"
    v3_checks = {
...
def main():
    ...
    out_path = V4 / "exports" / "analysis_report.json"
