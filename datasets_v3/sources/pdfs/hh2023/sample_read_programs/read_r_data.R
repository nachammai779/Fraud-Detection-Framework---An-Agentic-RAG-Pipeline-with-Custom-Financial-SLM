...
levels(metadata$VariableName)<-tolower(trim(levels(as.factor(metadata$VariableName))))
...
for (i in levels(metadata$VariableName)[levels(as.factor(metadata$VariableName)) %in% names(hh)] )
{
                print(i)
...
hh<-read.csv("hh2023_analys.csv",na.strings=".")
hh=applyFormats(hh)
...
svyby(~hunbnk,~hincvol,hh_svy,svyciprop,vartype="ci",level=.9)
svyby(~hbnkaccm,~hincvol,hh_svy,svyciprop,vartype="ci",level=.9)
svyby(~hbranchstatv2,~hincvol,hh_svy,svyciprop,vartype="ci",level=.9)
svyby(~huse12mo ,~hincvol,hh_svy,svyciprop,vartype="ci",level=.9)
svyby(~huse12cc,~hincvol,hh_svy,svyciprop,vartype="ci",level=.9)
...
hh_svy$variables$praceeth3=relevel(hh_svy$variables$praceeth3, ref='White')
summary(svyglm(husenowops=='Yes'~praceeth3, family=binomial, design=subset(hh_svy, praceeth3 %in% c('White','Black'))))
